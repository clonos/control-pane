...
foreach($nodes as $node){
	...
	if(!$db1->error){
		...
		foreach($bases as $base){
			...
			ClonOS::syslog("bases a.json.php base loop: base id=". $base['idx']);
			$idle = 1;
			...
			if($hres !== false){
				...
				ClonOS::syslog("bases a.json.php base loop: hres != false: ". implode(",", $vars));
				$html_tpl=$hres[1];

				foreach($vars as $var => $val){
					ClonOS::syslog("bases a.json.php replace [#".$var."#] by val: ".$val." hres = ". $hres[1]);
					$html_tpl = str_replace('#'.$var.'#', $val, $html_tpl);
//					$html .= $html_tpl;
					ClonOS::syslog("bases a.json.php replace result: ".$html_tpl);
				}
				$html .= $html_tpl;
			}
